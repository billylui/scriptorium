...
- **B — fail-closed (recommended for this gate).** Per term, put today's `\b` in front only if the term's first character is a word character that is not a combining mark, and `\b` after only if its last character is; a punctuation-edged or mark-edged side gets no boundary at all. Join the bounded terms with `|` under `(?i)`. Every edge B keeps is **the same `\b` assertion** today's pattern uses, and every edge it drops only removes a constraint, so B matches a **superset** of today's pattern by construction. Keep `\b` itself rather than rewriting kept edges as `(?<!\w)`/`(?!\w)`: under `(?i)` a term character and the text character it matches can differ in word-ness (Greek iota matches U+0345 COMBINING GREEK YPOGEGRAMMENI, which is not a word character), and the lookaround rewrite then unblocks text `\b` blocks today — e.g. the term `ται` against `τα` + U+0345 + `x`. B can over-block (e.g. `नमस्ते` inside `नमस्तेजी`) but never unblocks anything that blocks today (a randomized comparison on 2026-09-17 of 200,000 term/text pairs over letters, digits, `_`, punctuation, combining marks including U+0345, Greek iotas and spaces found 0 cases where today's pattern matched and B did not) — including inflected forms where a suffix vowel sign follows a name, such as the Bengali possessive `রাজেশের` ("Rajesh's") for the term `রাজেশ`, or `Zoe` + U+0308 + `y` (a decomposed Zoëy) for the term `Zoe` + U+0308.
