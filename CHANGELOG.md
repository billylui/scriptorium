...
- **Invisible characters in `.leakterms.local` counted as deny terms or broke real ones.** `strip()` removes whitespace but not Unicode format characters — a byte-order mark, zero-width spaces and joiners, bidi marks, soft hyphens. One at the start of a comment turned it into a "term", so a denylist checking no names reported one and suppressed the warning; one next to a real name meant that name never matched. Every denylist token is now trimmed of whitespace and any format character (by Unicode category, not a fixed list) at both ends, including the path and label of `!` exemptions. Characters inside a term are left as written. Ordinary denylists behave exactly as before — in particular an exemption with spaces around its `:` stays inert rather than starting to allow a file.
